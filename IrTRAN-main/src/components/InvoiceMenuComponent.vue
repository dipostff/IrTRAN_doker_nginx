...
function loadList() {
    invoiceList.value = getStoredList();
}

function openDocument(id) {
    router.push("/invoice/create/" + id);
}
...
<template>
    <div class="content-container">
        ...
        <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#invoiceSearchBackdrop">Найти документ</button>
        <div class="modal fade" id="invoiceSearchBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="invoiceSearchLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: #7DA5F0;">
                        <h1 class="modal-title fs-5" id="invoiceSearchLabel" style="color: white;">Поиск накладной</h1>
                    ...
                    <div class="modal-body">
                        <div class="container-fluid">
                            <div class="row">
                                ...
                                <div class="col-7">
                                    <input v-model="searchNumber" type="text" class="form-control" style="height: 25px; width: 270px;" placeholder="Введите ID накладной" />
                                </div>
                            </div>
            ...
            <table class="table table-hover table-bordered border-white">
                ...
                <tbody class="table-group-divider">
                    <tr v-if="invoiceList.length === 0">
                        <td colspan="13" class="text-center text-muted py-4">Нет сохранённых накладных. Создайте документ по кнопке «Создать документ» или найдите по номеру.</td>
                    </tr>
                    <tr
                        v-for="doc in invoiceList"
                        :key="doc.id"
                    ...
                    >
                        ...
                        <td>{{ listsStore.send_types[doc.id_send_type]?.name ?? '—' }}</td>
                        <td>{{ listsStore.legal_entities[doc.id_shipper]?.name ?? '—' }}</td>
                        <td>{{ listsStore.legal_entities[doc.id_receiver]?.name ?? '—' }}</td>
                        <td>—</td>
                        <td>{{ listsStore.stations[doc.id_station_departure]?.name ?? '—' }}</td>
                        <td>—</td>
                        <td>{{ listsStore.stations[doc.id_station_destination]?.name ?? '—' }}</td>
                        <td>—</td>
                        <td>{{ listsStore.countries[doc.id_place_of_payment]?.name ?? '—' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
