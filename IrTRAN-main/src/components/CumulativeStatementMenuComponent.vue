...
import { getLegalEntities } from "@/helpers/API";
...
function openDocument(id) {
    router.push("/cumulative-statement/create/" + id);
}
...
<template>
    <div class="content-container">
        ...
        <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#cumulativeStatementSearchBackdrop">Найти документ</button>
        <div class="modal fade" id="cumulativeStatementSearchBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="cumulativeStatementSearchLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: #7DA5F0;">
                        <h1 class="modal-title fs-5" id="cumulativeStatementSearchLabel" style="color: white;">Поиск накопительной ведомости</h1>
            ...
            <table class="table table-hover table-bordered border-white">
                ...
                <tbody class="table-group-divider">
                    <tr v-if="statementList.length === 0">
                        <td colspan="9" class="text-center text-muted">Нет сохранённых накопительных ведомостей. Создайте документ по кнопке «Создать документ» или найдите по номеру.</td>
                    </tr>
                    <tr v-for="doc in statementList" :key="doc.id" @dblclick="openDocument(doc.id)" style="cursor: pointer;">
                        ...
                        <td>Накопительная ведомость</td>
                        <td>{{ listsStore.legal_entities[doc.id_carrier_org]?.name ?? '—' }}</td>
                        <td>{{ doc.place_of_calculation || '—' }}</td>
                        <td>{{ listsStore.legal_entities[doc.id_payer]?.name ?? '—' }}</td>
                        <td>Безналичный централизованный</td>
                        <td>{{ doc.total_to_pay != null ? doc.total_to_pay : '—' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
