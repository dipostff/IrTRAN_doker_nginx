...
import { getStations } from "@/helpers/API";
...
function openDocument(id) {
    router.push("/act/common/create/" + id);
}
...
<template>
    <div class="content-container">
        ...
        <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#commonActSearchBackdrop">Найти документ</button>
        <div class="modal fade" id="commonActSearchBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="commonActSearchLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: #7DA5F0;">
                        <h1 class="modal-title fs-5" id="commonActSearchLabel" style="color: white;">Поиск акта (общей формы)</h1>
            ...
            <table class="table table-hover table-bordered border-white">
                <thead style="background-color: #7DA5F0; color: white;">
                    <tr>
                        ...
                        <th>Тип акта</th>
                        <th>Наим. станции составления акта</th>
                        <th>Кем составлен</th>
                ...
                <tbody class="table-group-divider">
                    <tr v-if="actList.length === 0">
                        <td colspan="7" class="text-center text-muted py-4">Нет сохранённых актов. Создайте документ или найдите по номеру.</td>
                    </tr>
                    <tr v-for="doc in actList" :key="doc.id" @dblclick="openDocument(doc.id)" style="cursor: pointer">
                        ...
                        <td>{{ doc.downtime_type === 'start' ? 'Начало простоя' : doc.downtime_type === 'end' ? 'Окончание простоя' : '—' }}</td>
                        <td>{{ listsStore.stations[doc.id_station]?.name ?? '—' }}</td>
                        <td>—</td>
                        <td>{{ (doc.wagons && doc.wagons.length) || 0 }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
